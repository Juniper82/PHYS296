...
for f,F in zip(range(len(pTdatmod)),pTdatmod):       
    for j,J in zip(range(len(valuedatmod)),valuedatmod):
        ...
        sub_ax = fig1.add_subplot(innerGrid[k]) # add subplot in innerGrid
        ...
        for z in range(len(zBin)-1): 
            ...
            if f != 0:
                sub_ax.errorbar(databin['pT'],databin['value'],yerr=databin['delta'],capsize=6,linestyle="",color = Zcolor[z])
                sub_ax.set_xlabel(r"$p_T$ (GeV)")
